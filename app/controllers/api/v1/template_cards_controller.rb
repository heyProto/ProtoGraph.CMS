    ...
    def index
        if @folder.vertical.present?
          if @folder.is_for_stories
            ...
            @template_cards = @account.template_cards.where(is_current_version: true).where.not(name: ['toImageNarrative', "toCoverImage", 'toVideo: Youtube', 'toQuiz', 'toTimeline', 'toExplain', 'toArticle', 'toVideo: JWPlayer', 'toProfile', 'toData: IRBF Grid', 'toData: IRBF Tooltip',"toHTML", "toDataWrapper", "toBio", "toCreditPartners"])
          end
        else
