class Api::V1::TemplateCardsController < ApiController
    ...
    def index
        @template_cards = @folder.vertical.present? ? @account.template_cards.where(is_current_version: true).where.not(name: 'toArticle') : @account.template_cards.where(is_current_version: true).where.not(name: ['toArticle', 'toStory'])
