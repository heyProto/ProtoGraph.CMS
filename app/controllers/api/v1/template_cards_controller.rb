class Api::V1::TemplateCardsController < ApiController
    ...
    def show
        ...
        if @template_card.present?
            render json: {template_card: @template_card.as_json(only: [:account_id, :id, :slug, :global_slug,:name, :elevator_pitch, :description, :template_datum_id, :git_repo_name], methods: [:account_slug, :files, :versions])}
        else
