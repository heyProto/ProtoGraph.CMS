class Api::V1::TemplateCardsController < ApiController
    ...
    def index
        if @folder.vertical.present?
          @template_cards = @account.template_cards.where(is_current_version: true).where(name: ['toQuiz', 'toTimeline', 'toCluster', 'toStory', 'ComposeCard', 'toMedia', 'toImage', 'VideoYoutube'])
        else
          @template_cards = @account.template_cards.where(is_current_version: true).where.not(name: ['toArticle', 'toStory', 'toCluster', 'toExplain', 'ComposeCard', 'toMedia', 'toImage', 'VideoYoutube'])
        end
