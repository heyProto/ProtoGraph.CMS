class ViewCastsController < ApplicationController
    ...
    def index
        ...
        @view_casts = @permission_role.can_see_all_view_casts ? @folder.view_casts.where.not(template_card_id: TemplateCard.to_story_cards_ids).where(is_autogenerated: false).order(updated_at: :desc).page(params[:page]).per(15) : current_user.view_casts(@folder).where(is_autogenerated: false).order(updated_at: :desc).page(params[:page]).per(15)
