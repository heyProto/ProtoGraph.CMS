...
class FeedLink < ApplicationRecord
  ...
  belongs_to :feed
  has_one :view_cast, class_name: "ViewCast", foreign_key: "id", primary_key: "view_cast_id"
  ...
  def create_or_update_view_cast
    ...
    if self.view_cast.present?
      ...
      view_cast.update({
        name: self.temp_headline.to_s || self.headline.to_s,
        seo_blockquote: "<blockquote><h4#>#{self.temp_headline.to_s || self.headline.to_s}</h4><p>#{self.link}</p></blockquote>", #TemplateCard.to_story_render_SEO(payload_json["data"]),
        folder_id: ref_category.folder.id,
  ...
  def create_datacast_json
    require 'uri'
    data = {"data" => {}}
    data["data"]["section"] = self.temp_headline.to_s || self.headline.to_s
    data["data"]["title"] = self.temp_headline.to_s || self.headline.to_s
