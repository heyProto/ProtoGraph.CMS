    ...
    def cards(apply_limit=true)
        if is_automated_stream
            if col_name == "Site"
                ...
                view_casts = site.view_casts.where(template_card_id: TemplateCard.where(name: "toStory").pluck(:id)).where(is_autogenerated: false).limit(self.limit).offset(self.offset).order(updated_at: :desc, created_at: :desc)
