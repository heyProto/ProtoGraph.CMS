...
class Page < ApplicationRecord
  ...
  before_update :assemble_cards, if: "self.prepare_cards_for_assembling=='true'"
  ...
  def assemble_cards
    ...
    view_cast_lists = []
    JSON.parse(self.content).each do |card|
      view_cast = ViewCast.where(:id => card["data-card-id"]).first
      # if card does not have a template or has a paragraph template,
      # proceed to associate new/existing Paragraph type ViewCast with it
      if card["data-template-id"].blank? || card["data-template-id"] == to_para_card.id
        para = card["htmlString"]
        ...
        # if ViewCast already exists, update it
        else
          view_cast = ViewCast.update(view_cast.id, {
            name: title,
            template_card_id: to_para_card.id,
            template_datum_id: to_para_schema.id,
            updated_by: updated_by,
